...
const StatusItem = ({ info }: Props): JSX.Element => {
  ...
  const addInfoItem = (e: any) => {
    ...
    addComponent([
      <ChatBotLayout>
        <ChatBotText>선택한 회의실의 예약 세부 정보를 확인하세요.</ChatBotText>
      </ChatBotLayout>,
      <ChatBotLayout>
  ...
  return (
    <Container onClick={addInfoItem}>
      <ContentWrapper>
        {/* <input type='checkbox'/> */}
        <Dday day={dDay(new Date(info.meetingDate!))}>
          {dDayStr(dDay(new Date(info.meetingDate!)))}
        </Dday>
        <Content>
          <InfoTable>
            <tbody>
              <tr>
                <Cell>
                  {new Date(info.meetingDate!).getFullYear() +
                    '-' +
                    (new Date(info.meetingDate!).getMonth() + 1) +
                    '-' +
...
const Dday = styled.span<{ day: number }>`
  ...
  background: ${props =>
    props.day >= 7
      ? 'gray'
      : props => (props.day >= 0 ? '#FFC3C3' : '#FD8E9E')};
`;
